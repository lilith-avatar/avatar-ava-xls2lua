            ...
            if (title_cell) {
                ...
                if (title_type != 's') {
                    return {
                        t: {},
                        ret: -1,
                        err_str: 'sheet[' + sheet_name + '] title column[' + (col_idx) + '] must be String'
                    }
                }
                if (type_type != 's') {
                    return {
                        t: {},
                        ret: -1,
                        err_str: 'sheet[' + sheet_name + '] type column[' + (col_idx) + '] must be String'
                    }
                }
                if (!types_check_list.includes(type_name)) {
                    return {
                        t: {},
                        ret: -1,
                        err_str: 'sheet[' + sheet_name + '] type column[' + (col_idx) + '] type wrong'
                    }
                }
            ...
            if (key_cell) {
                ...
                if (!worksheet[col_title + '2'] || !worksheet[col_title + '3']) {
                    return {
                        t: {},
                        ret: -1,
                        err_str: 'sheet[' + sheet_name + '] title and type of column[' + (col_idx) + '] must be String'
                    }
                }
...
async function convertToLua(input_path = INPUT_FOLDER, output_path = OUTPUT_FOLDER) {
    ...
    for (let xls_file of exl_list) {
        ...
        let {
            ...
            ret,
            err_str
        } = make_table(input_path + '/' + xls_file)
        file_counter += 1
