...
/* Not sure if this is needed since this is for qt UI msg
    ...
    for (let sheet of sheet_names) {
        ...
        for (let col_idx=1; col_idx<=column_count; col_idx++) {
            ...
            if (title_cell) {
                ...
                // condition check: type cell not empty. Not sure if this is needed 
                let type_name = String(type_cell.v).toLowerCase()
                let type_type = type_cell.t 
                // condition check if (title==null) deleted due to redundancy
                // 检查标题数据格式
                if (title_type != 's') {
        ...
        for (let row_idx=5; row_idx<=row_count; row_idx++) {
            ...
            for (let col_idx=1; col_idx<=column_count; col_idx++) {
                ...
                if (value_cell) {
                    ...
                    // value_cell为null时仍应写入data，值为nil
                    v = 'nil'
                }
                row[title] = v
...
function write_to_lua_script (workbook, output_path, xls_file) {
    for (let sheet_name in workbook['data']) {
        ...
        var outfp = fs.createWriteStream(path_filename, {
            flags: 'w' // 'a' means appending 即在文件结尾写入内容
        })
...
// let {t, ret, err_str} = read_file('./xls/GlobalSetting.xlsx')
// console.log('t', t, 'ret', ret, 'err_str', err_str)
