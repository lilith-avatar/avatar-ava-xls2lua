    ...
    for (let sheet of sheet_names) {
        ...
        for (let col_idx=1; col_idx<=column_count; col_idx++) {
            ...
            if (key_cell) {
                ...
                // 检查项：如果存在key关键字，则列名和类型不能为空
                if (!worksheet[col_title+'2'] || !worksheet[col_title+'3']) {
                    return {t: {}, ret: -1, err_str:'sheet['+sheet_name+'] title and type of column['+(col_idx)+'] cannot be empty'}
                }
            ...
            for (let col_idx=1; col_idx<=column_count; col_idx++) {
                ...
                if (value_cell) {
                    ...
                    if (type_checker[vtype].includes(type_dict[title])) {
                        ...
                        return {t: {}, ret: -1, err_str:'sheet['+sheet_name+'] column['+(col_idx)+'] row['+(row_idx)+'] data type error'}
                    }
                } else {
                    v = 'nil'
                }
...
/* Vector, Euler, Color等特殊类型的lua化处理 */
...
async function convertToLua (input_path=INPUT_FOLDER, output_path=OUTPUT_FOLDER) { 
    ...
    fs.readdirSync(input_path).forEach(file => {
        if (FILE_EXTENSIONS.includes(file.split('.').pop()) && !file.startsWith('~$')) exl_list.push(file)
    })
    // move translate excel to last element
    if (exl_list.includes(TRANSLATE_XLS)) {
        let op_index = exl_list.indexOf(TRANSLATE_XLS) 
        let last_element = exl_list.length-1
        exl_list[op_index] = exl_list[last_element]
        exl_list[last_element] = TRANSLATE_XLS 
    }
    ...
    let return_msgs = {
        ...
        file_list: exl_list, // 数组，已处理的文件列表
